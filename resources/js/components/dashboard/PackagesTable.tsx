...
const PackagesTable = () => {
  const { packages, filters, flash } = usePage().props as unknown as PackagesPayload;
  ...
  const getStatusBadge = (pkg: Package) => {
    if (pkg.is_featured) {
      return (
        <Badge variant="secondary" className="bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-100 text-xs">
          <Star className="w-3 h-3 mr-1" />
          Featured
        </Badge>
      );
  ...
  const renderImagesColumn = (media: PackageMedia[]) => {
    if (!media || media.length === 0) {
      return (
        <div className="flex items-center justify-center w-16 h-12 bg-gray-100 dark:bg-gray-700 rounded border-2 border-dashed border-gray-300 dark:border-gray-600">
          <span className="text-xs text-gray-400 dark:text-gray-300">No images</span>
        </div>
      );
    }
    const displayImages = media.slice(0, 2);
    ...
    return (
      <div className="flex items-center space-x-1">
        ...
        {remainingCount > 0 && (
          <div className="flex items-center justify-center w-10 h-10 bg-gray-100 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-600">
  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white">Travel Packages</h1>
          <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">Manage packages ({meta.total} total)</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
          ...
          >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="w-full sm:w-auto dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                  <Filter className="w-4 h-4 mr-2" />
                  Filters
                  <ChevronDown className="w-4 h-4 ml-2" />
                </Button>
              ...
              <DropdownMenuContent className="w-56 dark:bg-gray-800 dark:border-gray-700">
                <DropdownMenuLabel className="dark:text-white">Show Advanced Filters</DropdownMenuLabel>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-2 sm:space-y-0 w-full lg:w-1/3">
            ...
            >
              ...
              <SelectContent className="dark:bg-gray-800 dark:border-gray-700">
                <SelectItem value="title|asc" className="dark:hover:bg-gray-700">Title (A → Z)</SelectItem>
                <SelectItem value="title|desc" className="dark:hover:bg-gray-700">Title (Z → A)</SelectItem>
                <SelectItem value="base_price|asc" className="dark:hover:bg-gray-700">Price (Low → High)</SelectItem>
                <SelectItem value="base_price|desc" className="dark:hover:bg-gray-700">Price (High → Low)</SelectItem>
                <SelectItem value="booking_start_date|asc" className="dark:hover:bg-gray-700">
                  Booking Start (Earliest → Latest)
                </SelectItem>
                <SelectItem value="booking_start_date|desc" className="dark:hover:bg-gray-700">
                  Booking Start (Latest → Earliest)
                </SelectItem>
              </SelectContent>
            </Select>
            ...
            {anyFilterActive && (
              <Button variant="outline" className="sm:ml-4 dark:bg-gray-700 dark:border-gray-600 dark:text-white" onClick={clearFilters}>
                Clear Filters
              </Button>
            )}
          </div>
        ...
        {showFilters && (
          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Destination */}
              <div className="relative">
                <Label htmlFor="destination-filter" className="text-sm font-medium dark:text-gray-300">
                  Destination
                </Label>
                <Input
                  id="destination-filter"
                  ...
                  value={destination}
                  onChange={(e) => setDestination(e.target.value)}
                  className="mt-2 w-full dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                />
                {destination && (
                  <button
                    type="button"
                    className="absolute right-3 top-[2.5rem] text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
                    onClick={() => setDestination("")}
                  >
                    <X className="w-4 h-4" />
                  </button>
              ...
              {/* Booking Date Range */}
              <div className="space-y-2">
                <Label className="text-sm font-medium dark:text-gray-300">Booking Date Range</Label>
                <div className="flex flex-col space-y-2">
                  <div className="relative">
                    <Input
                      type="date"
              ...
              {/* Activities IDs */}
              <div className="relative">
                <Label htmlFor="activities-filter" className="text-sm font-medium dark:text-gray-300">
                  Activities (IDs)
                </Label>
                <Input
                  id="activities-filter"
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Enter comma-separated activity IDs</p>
              </div>
            </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="w-full overflow-x-auto">
          <table className="table-auto min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {packageData.map((pkg) => (
                <tr key={pkg.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                  {/* Images */}
                  <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                    {renderImagesColumn(pkg.media)}
                  </td>
                  ...
                  <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                    <div>
                      ...
                      <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 truncate max-w-[100px] sm:max-w-xs">
                        {pkg.description}
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <MapPin className="w-4 h-4 text-red-500 mr-1 sm:mr-2" />
                      <span className="text-xs sm:text-sm text-gray-900 dark:text-white">{pkg.location}</span>
                    </div>
                  </td>
                  ...
                  {/* Visibility */}
                  <td className="hidden md:table-cell px-3 sm:px-6 py-4 whitespace-nowrap">
                    <Badge variant="outline" className="capitalize text-xs dark:bg-gray-700 dark:text-white">
                      {pkg.visibility}
                    </Badge>
                  </td>
                  ...
                  <td className="hidden md:table-cell px-3 sm:px-6 py-4 whitespace-nowrap">
                    <Badge
                      variant={pkg.is_refundable ? "default" : "secondary"}
                      className={`text-xs ${pkg.is_refundable ? "bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-100" : ""}`}
                    >
                      {pkg.is_refundable ? "Yes" : "No"}
                    </Badge>
                  </td>
                  ...
                  <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                    {pkg.flight_from && pkg.flight_to ? (
                      <div className="text-xs sm:text-sm text-gray-900 dark:text-white">
                        {pkg.flight_from.toUpperCase()} → {pkg.flight_to.toUpperCase()}
                        {pkg.booking_class && pkg.airline_name && (
                          <span className="block text-gray-500 dark:text-gray-400 text-[10px] sm:text-xs">
                            (class: {pkg.booking_class}, airline: {pkg.airline_name})
                          </span>
                        )}
                      </div>
                  ...
                  <td className="hidden lg:table-cell px-3 sm:px-6 py-4 whitespace-nowrap">
                    {pkg.hotel_name ? (
                      <div className="text-xs sm:text-sm text-gray-900 dark:text-white">
                        {pkg.hotel_name}{" "}
                        {pkg.hotel_star_rating !== null && (
                          <span className="ml-1 text-yellow-600 dark:text-yellow-400 text-[10px] sm:text-xs">
                            ★{pkg.hotel_star_rating}
                          </span>
                        )}
                        {pkg.hotel_checkin && pkg.hotel_checkout && (
                  ...
                  <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="sm" className="h-8 px-2 sm:px-4 dark:text-white">
                          <span className="hidden sm:inline">Actions</span>
                          <ChevronDown className="w-4 h-4 sm:ml-1" />
                        </Button>
                      </DropdownMenuTrigger>
        ...
        {packageData.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">No packages found</h3>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Try adjusting your filters or search.</p>
          </div>
        )}

        {/* Pagination */}
        {meta.last_page > 1 && (
          <div className="bg-white dark:bg-gray-800 px-4 py-3 flex flex-col sm:flex-row items-center justify-between border-t border-gray-200 dark:border-gray-700 sm:px-6">
            {/* Mobile: Previous / Next */}
            <div className="flex flex-1 justify-between w-full sm:hidden">
              <Button
                variant="outline"
              ...
              <div>
                <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                  {meta.links.map((link, idx) => {
                    ...
                    if (!link.url) {
                      return (
                        <Button key={idx} variant="outline" size="sm" disabled className="dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                          {labelText}
                        </Button>
                      );
                    }
                    return (
                      <Button
                        ...
                        size="sm"
                        onClick={() => goToPage(link.url)}
                        className={link.active ? "" : "dark:bg-gray-700 dark:border-gray-600 dark:text-white"}
                      >
                        {labelText}
                      </Button>
                    );
      ...
      >
        <AlertDialogContent className="dark:bg-gray-800 dark:border-gray-700">
          <AlertDialogHeader>
            <AlertDialogTitle className="dark:text-white">Delete Package</AlertDialogTitle>
            <AlertDialogDescription className="dark:text-gray-300">
              Are you sure you want to delete this package? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex-col space-y-2 sm:flex-row sm:justify-end sm:space-x-2 sm:space-y-0">
            <AlertDialogCancel className="w-full sm:w-auto dark:bg-gray-700 dark:border-gray-600 dark:text-white">Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={confirmDelete}
              className="w-full sm:w-auto bg-red-600 hover:bg-red-700 dark:bg-red-700 dark:hover:bg-red-800 dark:text-white"
            >
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
