...
export default function CreateActivity() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const formattedData = {
      ...
      time_slots: data.time_slots
        .filter((slot) => slot.starts_at && slot.ends_at)
        .map((slot) => ({ starts_at: slot.starts_at, ends_at: slot.ends_at })),
    };

    post(route('activities.store'), { data: formattedData });
  };
    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 dark:bg-gray-900">
        ...
        <div className="flex items-center justify-between p-4">
          <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">Create Activity</h2>
        </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    value={data.price}
                    onChange={(e) => setData('price', e.target.value)}
                    className="w-full h-10 pl-8 pr-3 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100"
                    required
                  />
                </div>
