...
export default function EditActivity({ activity }: { activity: Activity }) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const validTimeSlots = data.time_slots.filter((slot) => slot.starts_at && slot.ends_at);
    
    if (validTimeSlots.length === 0) {
      // Handle case where no valid time slots exist
      return;
    }
    ...
    const formattedData = {
      ...
      // Don't include price since it's not editable
      time_slots: validTimeSlots.map((slot) => ({ 
        id: slot.id, 
        starts_at: slot.starts_at, 
        ends_at: slot.ends_at 
      })),
    ...
    put(route('activities.update', { activity: activity.id }), formattedData);
  };
  ...
  const validateTimeSlot = (slot: TimeSlot) => {
    if (!slot.starts_at || !slot.ends_at) return true; // Allow empty slots
      ...
      <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 dark:bg-gray-900">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          <div className="border-t border-gray-200 dark:border-gray-600 pt-4 sm:pt-6">
            ...
            {data.time_slots.map((slot, index) => {
              const isValidSlot = validateTimeSlot(slot);
              return (
                <div
                  key={slot.id}
                  className="p-4 border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 mb-4 dark:bg-gray-700"
                >
                  ...
                  {!isValidSlot && slot.starts_at && slot.ends_at && (
